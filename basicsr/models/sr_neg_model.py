...
class SRNegModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SRNegModel, self).__init__(opt)

        # define network
        self.net_g = build_network(opt['network_g'])
        self.net_g = self.model_to_device(self.net_g)
        if opt.get('complie', False):
        ...
        if self.opt.get('network_g_net', None):
            self.net_g_neg = build_network(
                self.opt['network_g_net']).to(self.device)
        else:
            self.net_g_neg = build_network(
                self.opt['network_g']).to(self.device)
            neg_start = self.opt['train'].get('neg_start', 0.999)
            self.model_neg(neg_start)  # copy net_g weight

        neg_load_path = self.opt['path_neg'].get(
            'pretrain_network_g_neg', None)
        if neg_load_path is not None:
            param_key = self.opt['path_neg'].get('param_key_g', 'params')
            self.load_network(self.net_g, load_path, self.opt['path_neg'].get(
                'strict_load_g', True), param_key)
    ...
    def model_neg(self, decay=0.999):
        net_g = self.get_bare_model(self.net_g)

        net_g_params = dict(net_g.named_parameters())
        net_g_ema_params = dict(self.net_g_neg.named_parameters())
    ...
    def init_training_settings(self):
        ...
        self.neg_decay = train_opt.get('neg_decay', 0)
        self.neg_iter = train_opt.get('neg_iter', 0)
    ...
    def optimize_parameters(self, current_iter):
        ...
        with torch.no_grad():
            neg_out = self.net_g_neg(self.lq)
        ...
        if self.cri_perceptual:
            l_percep, l_style = self.cri_perceptual(self.output, neg_out)
        ...
        if self.cri_contrastive:
            l_contras = self.cri_contrastive(self.output, neg_out, self.gt)

            l_total += l_contras
            loss_dict['l_contras'] = l_contras
        ...
        if current_iter % self.neg_iter == 0:
            self.model_neg(decay=self.neg_decay)

    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                if self.opt['val'].get('ensemble', False):
                    out = []
                    for i in range(4):
                        sr_tmp = self.net_g_ema(
                            torch.rot90(self.lq, i, [-2, -1]))
                        out.append(torch.rot90(sr_tmp, -i, [-2, -1]))
                    self.output = torch.cat(
                        out, dim=0).mean(dim=0, keepdim=True)
                else:
            ...
            with torch.no_grad():
                if self.opt['val'].get('ensemble', False):
                    out = []
                    for i in range(4):
                        sr_tmp = self.net_g(torch.rot90(self.lq, i, [-2, -1]))
                        out.append(torch.rot90(sr_tmp, -i, [-2, -1]))
                    self.output = torch.cat(
                        out, dim=0).mean(dim=0, keepdim=True)
                else:
...
@MODEL_REGISTRY.register()
class SwinIRNegModel(SRNegModel):
    def __init__(self, opt):
        super().__init__(opt)
