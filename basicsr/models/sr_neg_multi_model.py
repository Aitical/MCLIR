...
class SRNegModel_Multi(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SRNegModel_Multi, self).__init__(opt)
    ...
    def model_neg(self, model, decay=0.999):
        net_g = self.get_bare_model(self.net_g)

        net_g_params = dict(net_g.named_parameters())
        net_g_ema_params = dict(model.named_parameters())
    ...
    def init_training_settings(self):
        ...
        self.neg_start = train_opt.get('neg_start', 0.01)
        self.neg_update_scaling = train_opt.get('update_scaling', 1.0)
        self.neg_num = len(self.neg_iter)
        logger = get_root_logger()
        logger.warning(f'Update Negative Model {self.neg_num}')

        self.net_g_neg = [build_network(self.opt['network_g']).to(
            self.device) for i in range(self.neg_num)]
        for net in self.net_g_neg:
            self.model_neg(net, self.neg_start)   # copy net_g weight
...
@MODEL_REGISTRY.register()
class SwinIRNegModel_Multi(SRNegModel_Multi):
    def __init__(self, opt):
        super().__init__(opt)
    ...
    def test(self):
        ...
        window_size = self.opt['network_g'].get('window_size', 8)
        temp_size = self.opt['val'].get('window_size', 0)
